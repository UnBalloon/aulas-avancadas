...
vector<pair<ll, ll>> tree(4*MAX, {LLINF, 1}), vet(MAX, {LLINF, 1}); // segtree e vetor
int N;
...
pair<ll, ll> query(int A, int B, int l=0, int r=N-1, int no=1){
    if(r<A or B<l) return {LLINF, 0}; // 1º caso
    if(A<=l and r<=B) return tree[no]; // 2º caso

    // 3º caso
    int mid = (l + r)/2;

    pair<ll, ll> pLeft = query(A, B, l, mid, 2*no);
    pair<ll, ll> pRight = query(A, B, mid+1, r, 2*no+1);
